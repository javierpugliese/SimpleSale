  ...
  <div class="attributes">
    <v-container>
      ...
      >
        <template v-slot:top>
          <v-toolbar flat>
            <v-toolbar-title>Lista de Atributos</v-toolbar-title>
            ...
            <v-dialog v-model="dialogDelete" max-width="500px">
              <v-card>
                <v-card-title class="headline"
                  >¿Está seguro de que quiere eliminar este atributo?
                </v-card-title>
                <v-card-actions>
                  <v-spacer></v-spacer>
...
export default {
  ...
  methods: {
    async initialize() {
      ...
      const attributes = this.$http.get("Atributos");
      const attributeTypes = this.$http.get("TiposDeAtributo");
      await Promise.all([attributes, attributeTypes])
        .then((responses) => {
          const attributesRes = responses[0];
          const attributeTypesRes = responses[1];

          if (attributesRes && attributesRes.data)
            this.attributes = attributesRes.data;
          if (attributeTypesRes && attributeTypesRes.data) {
            this.attributeTypes = attributeTypesRes.data.map((at) => ({
              text: at.nombre,
              value: at.id,
            }));
          }
    ...
    editItem(item) {
      ...
      this.editedItem.idTipo = item.idTipo || -1;
      this.dialog = true;
    },
    ...
    async deleteItemConfirm() {
      this.loading = true;
      await this.$http
        .delete(`Atributos/${this.editedId}`, this.editedItem)
        .then((res) => {
          if (res) {
    ...
    async save() {
      ...
      if (this.editedIndex > -1 && this.editedId > -1) {
        ...
        await this.$http
          .put(`Atributos/${this.editedId}`, this.editedItem)
          .then((res) => {
            if (res) {
              this.snackbarText = "Operación realizada exitosamente.";
              this.snackbarColor = "success";
        ...
        await this.$http
          .post("Atributos", this.editedItem)
          .then((res) => {
            if (res) {
              this.snackbarText = "Operación realizada exitosamente.";
              this.snackbarColor = "success";
